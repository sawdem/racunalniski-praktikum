...
\[\text{velikostTabele}\equiv \text{številoUrNaDan} \cdot \text{številoDelovnihDni}\]
...
predstavljen z množico \textit{srečanj}. Posamezno srečanje je sestavljeno iz predmeta,
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   \begin{enumerate}
      \item Z zanko se sprehodimo po vektorju vseh predmetov. Predmet na \textit{\(\text{i}\)}-tem koraku
      označimo s \(\text{predmet}_i\).
   
      \item Naslednja vgnezdena zanka nas popelje po vseh predavalnicah. Predavalnico na
    \textit{  \(\text{j}\)}-tem koraku označimo s \(\text{predavalnica}_j\).
...
\textit{Operacija} je vsaka sprememba oziroma poseg v urnik med izvajanjem optimizacije.
Na primeru urnika si oglejmo, katere so smiselne operacije:

% Vsaka od vrstic v okolju `itemize` je en element naštevanja.
% začetek okolja itemize
   \begin{itemize}
   \item \textit{Vstavljanje}  srečanja v urnik.
   \item \textit{Brisanje}  srečanja iz urnika.
   \item \textit{Zamenjava}  dveh srečanj v urnik.
   \end{itemize}
% konec okolja itemize
% Spodnja vrstica ne začne novega odstavka, zato ne bi smela biti zamaknjena.
% To popravite z ukazom `\noindent` ali pa tako, da odstranite prazno vrstico.
% (Nekoliko lepše izgleda, če v prazno vrstico vstavite % za komentar.)
\textit{Okolica} operacije je množica vseh možnih sprememb, ki jih lahko s to operacijo
...
\textit{Sistem okolic} je množica vseh možnih sprememb, ki jih lahko z vsemi operacijami iz
...
Rešitev se ponuja kar sama--za naslednjih \textit{n} korakov prepovemo vsako spremembo, ki jo
opravimo v koraku optimizacije. Izbira konstante \textit{n} je odvisna od samega problema, saj
ne obstaja neki splošen \textit{n}, ki bi nas v vsakem primeru pripeljal iz lokalnega ekstrema.
Imamo torej seznam prepovedanih sprememb--\textit{tabujev} in na vsakem koraku za izbrano
...
Naj bo \(\textit{n}--100\) ter \(\text{s}_1\) sprememba urnika, ki nas je pripeljala v lokalni maksimum. To
spremembo (npr. zamenjava  \(\text{srečanja}_i\)in \(\text{srečanja}_j\)) sedaj shranimo v
seznam tabujev. V naslednjem koraku izvedemo spremembo \(\text{s}_1\), ki se prav tako shrani v
seznam prepovedanih sprememb. Podobno naredimo za vse spremembe v nadaljnih korakih. S
tem poskrbimo, da se lahko v isti lokalni maksimum vrnemo šele čez \(\textit{n}--100\) korakov, saj
se sprememba \(\text{s}_1\), ki vodi do lokalnega maksimuma, šele tedaj izbriše iz seznama
prepovedanih sprememb.

Če smo dosegli lokalni maksimum in vrednost kriterijske funkcije v \textit{n} korakih uspemo
...
% konec okolja verbatim
   ...
   \begin{enumerate}
      ...
      \item Rešitev \textit{r}, ki smo jo dobili s pomočjo požrešne metode, izboljšamo z lokalnim vzpenjanjem
      ...
      \item Na vsakem koraku ocenimo izboljšamo rešitev \textit{r}. Če je vrednost kriterijske funkcije
      za trenutno rešitev prešla zastavljeno zadovoljivo mejo, potem se program zaključi. Prav tako
      se zaključi, če po \textit{n} korakih še vedno tičimo v istem lokalnem maksimumu.
